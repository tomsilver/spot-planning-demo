...
from spot_planning_demo.envs.spot_pybullet_env import SpotPyBulletSim, ActionFailure
...
def test_spot_pybullet_move_base():
    """Tests for MoveBase()."""
    sim = SpotPyBulletSim(use_gui=False,
                          raise_error_on_action_failures=True)  # change use_gui to True for debugging
    sim.reset(seed=123)
    ...
    move_into_table = MoveBase(Pose((table_pose.position[0], table_pose.position[1], 0)))
