...
from spot_planning_demo.spot_utils.skills.spot_hand_move import close_gripper, stow_arm
from spot_planning_demo.spot_utils.utils import get_robot_state
...
def grasp_at_pixel(
    ...
    """Grasp an object at a specified pixel in the RGBD image, which should be from the
    hand camera and should be up to date with the robot's state.
    ...
    if grasp_rot is not None:
        ...
        grasp.grasp_params.grasp_params_frame_name = (  # pylint: disable=no-member
            VISION_FRAME_NAME
        )
        ...
        # Turn into a proto.
        constraint = (
            grasp.grasp_params.allowable_orientation.add()  # pylint: disable=no-member
        )
