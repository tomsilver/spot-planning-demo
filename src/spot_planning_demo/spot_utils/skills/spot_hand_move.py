...
def move_hand_to_relative_pose_with_velocity(
    ...
    """Move the spot hand with a certain velocity specified as a duration (so velocity
    will become 1/duration).
    ...
    while True:
        ...
        if (
            feedback_resp.feedback.synchronized_feedback.arm_command_feedback.arm_cartesian_feedback.status  # pylint: disable=line-too-long
            in [
                arm_command_pb2.ArmCartesianCommand.Feedback.STATUS_TRAJECTORY_COMPLETE,  # pylint: disable=no-member
                arm_command_pb2.ArmCartesianCommand.Feedback.STATUS_TRAJECTORY_STALLED,  # pylint: disable=no-member
            ]
        ):
            break
        time.sleep(0.1)
...
def stow_arm(robot: Robot, timeout: float = 5) -> None:
    ...
    # Enable stowing.
    override = manipulation_api_pb2.ApiGraspedCarryStateOverride(override_request=3)
...
if __name__ == "__main__":
    ...
    def _run_manual_test() -> None:
        ...
        resting_pose = math_helpers.SE3Pose(
            x=0.80, y=0, z=0.45, rot=math_helpers.Quat()
        )
