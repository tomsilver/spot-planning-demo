...
The main point of this code is to provide a consistent world frame within and between
runs. That way, absolute positions of objects that are saved in task files remain
accurate, even when starting the robot from a different location.

Before using this interface, follow instructions from the 'Mapping' section of the
README file under the spot_utils folder.
"""
...
class SpotLocalizer:
    ...
    def localize(self, num_retries: int = 10, retry_wait_time: float = 1.0) -> None:
        ...
        It's good practice to call this periodically to avoid drift issues. April tags
        need to be in view.
        """
        try:
